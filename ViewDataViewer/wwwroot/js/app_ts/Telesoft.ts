...
export class Downloads {
    private readonly threeQuarters = 0b1111101;
    private readonly space = 0b0100000;
    private readonly bar = 0b1111100;
    private readonly outputBuffer = new OutputBuffer();
    private state = 0;
    private frameNumber = 0;
    //  xor of bytes in frame between |A and |Z
    private runningChecksum = 0;
    //  the expected checksum
    private checksum = 0;
    //  a..z
    private frameLetter: string | null = null;
    ...
    constructor(
        ...
        private readonly onEndOfFrame?: (frameLetter: string) => void,
        private readonly onEndOfFile?: (filename: string, size: number) => void) {
    }
    ...
    TryDecodeHeaderFrame(b64: string): boolean {
    ...
    Decode(b64: string, invokeCallbacks = true, checkParity = false): number {
        const buffer = Uint8Array.from(atob(b64), c => c.charCodeAt(0));
       
        for (let bufferIdx = 0; bufferIdx < buffer.length; ++bufferIdx) {
